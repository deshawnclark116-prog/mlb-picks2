#!/usr/bin/env python3
"""
CFB_FCS_PASSING_YARDS_CHAMPION_GATE_A

FCS analog of the FBS passing_yards champion gate. DEV=2022-2023 train,
VAL_SEASON=2024, HOLDOUT_SEASON=2025 untouched. Same pre-registered bar.

Run
---
python -u cfb_fcs_passing_yards_champion_gate_a.py
"""
...
BASELINE_DEFAULT = "/data/cfb_model/cfb_fcs_passing_yards_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "/data/cfb_model/cfb_fcs_passing_yards_champion_gate_a_work"
...
def load(baseline_path):
    ...
    rows = con.execute(f"SELECT {', '.join(cols)} FROM cfb_fcs_passing_yards_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("CFB_FCS_PASSING_YARDS_CHAMPION_GATE_A\n======================================")
    ...
    (work / "cfb_fcs_passing_yards_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "CFB_FCS_PASSING_YARDS_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
              "constant": constant, "challenger": challenger, "gate": GATE,
              "passed": passed, "verdict": verdict, "importance": imp,
              "best_iteration": bst.best_iteration}
    (work / "cfb_fcs_passing_yards_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

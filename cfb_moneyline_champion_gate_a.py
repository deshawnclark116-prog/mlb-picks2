#!/usr/bin/env python3
"""
CFB_MONEYLINE_CHAMPION_GATE_A

Real team-level "who wins" classifier, mirroring the same proven
champion-gate process every other market in this repo goes through:
regularized XGBoost binary:logistic, DEV/VAL/HOLDOUT season splits,
power-adjusted bootstrap calibration test, pre-registered 4-bar pass gate.

DEV=2018-2022, VAL=2023, HOLDOUT=2024 -- same split as cfb_anytime_
touchdowns (2025+ reserved for live serving; unlike anytime_touchdowns,
2025 isn't excluded from a completeness problem here -- final scores are
complete every season -- it's just held out for symmetry/consistency with
the rest of this repo's convention).

Pre-registered pass bar (all), identical to every other CFB market's gate:
  1. AUC >= 0.58
...
Run
---
python -u cfb_moneyline_champion_gate_a.py
"""
...
BASELINE_DEFAULT = "cfb_models/cfb_moneyline_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "cfb_models/cfb_moneyline_champion_gate_a_work"
...
def load(baseline_path):
    ...
    cols = ["season", "week"] + FEATURES + ["team_won"]
    rows = con.execute(f"SELECT {', '.join(cols)} FROM cfb_moneyline_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("CFB_MONEYLINE_CHAMPION_GATE_A\n=============================")
    ...
    verdict = ("CFB_MONEYLINE_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "CFB_MONEYLINE_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    bst.save_model(str(work / "cfb_moneyline.json"))
    (work / "cfb_moneyline_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "CFB_MONEYLINE_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
    ...
    (work / "cfb_moneyline_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

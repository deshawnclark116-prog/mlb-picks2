...
set), same pre-registered bar -- NOT loosened for the smaller sample,
per this repo's standing rule against relaxing a gate to force a pass.

Real constraint, disclosed in the baseline's docstring: only one usable
dev season exists (2024, n=131 after eligibility -- 2022/2023 have too
little box-score coverage to train on). No separate internal-val season
like the FBS build had (2024), so train/internal-val is an 80/20 split
of the 2024 pool itself (early-stopping guard only, not a second
holdout). HOLDOUT_SEASON=2025 (n=341) is untouched until final scoring,
...
def main():
    ...
    print(f"train {DEV_SEASONS} (80% split) rows: {len(tr)}   internal val (20% split) rows: {len(va)}   "

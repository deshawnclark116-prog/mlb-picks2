#!/usr/bin/env python3
"""
CFB_FCS_PASSING_TOUCHDOWNS_CHAMPION_GATE_A

FCS analog of the FBS passing_touchdowns champion gate (which passed
cleanly on the FBS side with no calibration issues at all -- unlike
rushing/receiving/passing_yards). DEV=2022-2023, VAL=2024,
HOLDOUT=2025 untouched. Same pre-registered bar.

Run
---
python -u cfb_fcs_passing_touchdowns_champion_gate_a.py
"""
...
BASELINE_DEFAULT = "/data/cfb_model/cfb_fcs_passing_touchdowns_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "/data/cfb_model/cfb_fcs_passing_touchdowns_champion_gate_a_work"
...
def load(baseline_path):
    ...
    rows = con.execute(f"SELECT {', '.join(cols)} FROM cfb_fcs_passing_touchdowns_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("CFB_FCS_PASSING_TOUCHDOWNS_CHAMPION_GATE_A\n===========================================")
    ...
    verdict = ("CFB_FCS_PASSING_TOUCHDOWNS_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "CFB_FCS_PASSING_TOUCHDOWNS_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    bst.save_model(str(work / "cfb_fcs_passing_touchdowns.json"))
    (work / "cfb_fcs_passing_touchdowns_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "CFB_FCS_PASSING_TOUCHDOWNS_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
              "constant": constant, "challenger": challenger, "gate": GATE,
              "passed": passed, "verdict": verdict, "importance": imp,
              "best_iteration": bst.best_iteration}
    (work / "cfb_fcs_passing_touchdowns_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

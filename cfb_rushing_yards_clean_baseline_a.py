#!/usr/bin/env python3
"""
CFB_RUSHING_YARDS_CLEAN_BASELINE_A

First CFB market, mirroring nfl_rushing_yards_clean_baseline_a.py exactly
(same strict-D1 discipline, same feature shapes) -- the user asked for the
CFB build to start the same way the NFL build did.

Builds a STRICT D-1 dataset from cfb_model.sqlite::player_games. Every
feature for a given player-game uses only that player's (and that
opponent's) games with a strictly earlier (season, week) in the same
season. Scoped to RB only, same reasoning as the NFL build: RBs are the
dominant, structurally homogeneous rushing population.

Eligible population: position == RB, >= 3 prior games, AND a rolling
recent (last 3 games) carries-per-game rate >= 12 -- verified empirically
on real data (n=2826, mean 77.9 / median 69.0 rush yards for that
population), same bar the NFL build used and landed on for the same
reason (ensures genuine current feature-back workload, not a committee
back).

Threshold selection: computes the actual_rushing_yards distribution on the
2022-2023 DEV seasons (never touches the 2025 holdout) across real
prop-line candidates and reports the threshold closest to a genuine 50/50
split.
...
--------
  ...
  opp_rush_yards_allowed_per_game   (as-of: mean rushing yards allowed by
                                      this opponent across ALL eligible RBs
                                      who have faced them so far this season)
...
Split: 2022-2023 = development, 2024 = internal val (early stopping only),
2025 = one-shot out-of-time holdout (full complete season).

Read-only on cfb_model.sqlite. Writes only its own baseline.sqlite + manifest.

Run
---
python -u cfb_rushing_yards_clean_baseline_a.py
"""
...
WORKDIR_DEFAULT = "/data/cfb_model/cfb_rushing_yards_clean_baseline_a_work"
MIN_PRIOR_GAMES_FOR_RATE = 3
MIN_RECENT_CARRIES_PER_GAME = 12
THRESHOLD_CANDIDATES = [49.5, 59.5, 69.5, 79.5, 89.5, 99.5]
...
def main():
    ...
    print("CFB_RUSHING_YARDS_CLEAN_BASELINE_A\n===================================")
    print(f"source={src}\nworkdir={work}")
    ...
    manifest = {
        "script": "CFB_RUSHING_YARDS_CLEAN_BASELINE_A",
    ...
    print(f"\nbaseline db: {base_db}")
    print("Read-only on cfb_model.sqlite. No production state changed.")
    return 0

...
Run:
    python hr_opponent_raw_audit_d.py --rebuild
"""
...
def parse_game_date_series(pd, s):
    raw = s.copy()

    # SQLite sometimes returns pandas/numpy datetime64 values as microsecond integers.
...
def load_df(pd, conn):
    ...
    df["game_date"] = parse_game_date_series(pd, df["game_date"])

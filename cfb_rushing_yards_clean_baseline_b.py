#!/usr/bin/env python3
"""
CFB_RUSHING_YARDS_CLEAN_BASELINE_B

gate_a/gate_b (Platt) both left rushing_yards short on calibration --
Platt had nothing to correct because the internal-val slice didn't share
the holdout's specific miscalibration. Different theory this time, not a
recalibration attempt: the original feature set (copied straight from the
NFL build) has no notion of TEAM STRENGTH or game competitiveness, which
matters far more in college football than the NFL -- FBS spans everything
from playoff-contending Power 4 programs to Group of 5 teams that lose by
40, and that mismatch drives real things a rushing prop cares about
(garbage-time bench rotation in blowouts, backups mopping up, whether the
starter even plays into the 4th quarter). None of that was in gate_a's
feature set at all.

Added here (all as-of, strictly prior games, computed from real final
scores already in games -- NOT odds, NOT a bookmaker line):
  team_net_margin   own team's season-to-date (points_for - points_against)
                     average per game so far
  opp_net_margin    opponent's own season-to-date net margin
  projected_margin  team_net_margin - opp_net_margin (a real point-margin-
                     projection proxy built from actual results, the closest
                     legitimate analog to a spread without touching odds)

Everything else (population, eligibility, strict-D1 discipline, dev/val/
holdout split) is identical to clean_baseline_a -- this is an additive
feature test, not a redesign.

Run
---
python -u cfb_rushing_yards_clean_baseline_b.py
"""
...
WORKDIR_DEFAULT = "/data/cfb_model/cfb_rushing_yards_clean_baseline_b_work"
...
def build_team_margin_asof(conn):
    """As-of (team, season, week) -> season-to-date net scoring margin,
    using only games with strictly earlier week in that season. Built
    directly from real final scores (games.home_points/away_points),
    never from odds."""
    games = conn.execute(
    ...
    team_state = {}   # (season, team) -> [points_for_sum, points_against_sum, n]
    margin_asof = {}  # (season, team, week) -> net margin as of before that week
...
def main():
    ...
    print("CFB_RUSHING_YARDS_CLEAN_BASELINE_B\n===================================")
    ...
    out.execute(f"""CREATE TABLE cfb_rushing_yards_baseline (
    ...
    ins = f"INSERT INTO cfb_rushing_yards_baseline ({', '.join(insert_cols)}) VALUES ({placeholder})"
    ...
    manifest = {
        "script": "CFB_RUSHING_YARDS_CLEAN_BASELINE_B",
        ...
        "new_features_vs_a": ["team_net_margin", "opp_net_margin", "projected_margin"],
    }

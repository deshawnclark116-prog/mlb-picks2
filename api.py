...
# NFL predictions are generated by a separate weekly pipeline
# (nfl_serving_builder_a.py, run by .github/workflows/nfl_weekly.yml) that
# writes straight to the repo, not to the /data volume -- this API reads
# that committed file rather than recomputing anything, same pattern
# REPO_MODEL_DIR already uses for repo-committed model artifacts.
REPO_NFL_PREDICTIONS_PATH = Path(__file__).resolve().parent / "docs" / "nfl_predictions.json"
# CFB predictions are generated by a separate weekly pipeline
...
REPO_TENNIS_RECORD_PATH = Path(__file__).resolve().parent / "docs" / "tennis_record.json"
# Tennis predictions are generated by tennis_serving_builder_a.py (run in
# the combined workflow after tennis_player_matches_foundation_a.py),
# same static-read pattern as the NFL/CFB paths above. Only total_games
# and set_betting ship -- total_aces, double_faults, moneyline, and
# games_spread all failed their champion gates and are not generated at
# all, so they'll never appear here.
REPO_TENNIS_PREDICTIONS_PATH = Path(__file__).resolve().parent / "docs" / "tennis_predictions.json"
# NHL predictions are generated by nhl_serving_builder_a.py (run in the
# combined workflow), same static-read pattern as the paths above. Only
# moneyline (+ its prior-season-informed early-season variant) ships --
# it's the first and, for now, only NHL market in this repo.
REPO_NHL_PREDICTIONS_PATH = Path(__file__).resolve().parent / "docs" / "nhl_predictions.json"
REPO_NHL_RECORD_PATH = Path(__file__).resolve().parent / "docs" / "nhl_record.json"
...
def nfl_predictions():
    ...
    if not REPO_NFL_PREDICTIONS_PATH.exists():
        return {"generated_at_utc": None, "season": None, "week": None,
                "markets": {}, "picks": []}
    doc = _sanitize_nan(json.loads(REPO_NFL_PREDICTIONS_PATH.read_text()))
...
def cfb_predictions():
    ...
    if not REPO_CFB_PREDICTIONS_PATH.exists():
        return {"generated_at_utc": None, "season": None, "week": None,
                "markets": {}, "picks": []}
    doc = _sanitize_nan(json.loads(REPO_CFB_PREDICTIONS_PATH.read_text()))
    generated_at = doc.get("generated_at_utc")
    for p in doc.get("picks") or []:
        p["prop_type"] = p.get("market")
        if p.get("model_prob") is not None:
...
@app.get("/tennis/predictions")
def tennis_predictions():
    """Serves the file tennis_serving_builder_a.py writes (see
    ...
    """
    if not REPO_TENNIS_PREDICTIONS_PATH.exists():
        return {"generated_at_utc": None, "markets": {}, "picks": []}
    doc = _sanitize_nan(json.loads(REPO_TENNIS_PREDICTIONS_PATH.read_text()))
    generated_at = doc.get("generated_at_utc")
    for p in doc.get("picks") or []:
        p["prop_type"] = p.get("market")
        p["generated_at"] = generated_at
...
@app.get("/nhl/predictions")
def nhl_predictions():
    """Serves the file nhl_serving_builder_a.py writes (see
    REPO_NHL_PREDICTIONS_PATH) -- read-only, no live compute here. Same
    normalization as /cfb/predictions and /nfl/predictions. Moneyline is
    team-level (no line/prob_over fields), same shape as CFB's own
    moneyline picks."""
    if not REPO_NHL_PREDICTIONS_PATH.exists():
        return {"generated_at_utc": None, "season": None, "game_date": None,
                "markets": {}, "picks": []}
    doc = _sanitize_nan(json.loads(REPO_NHL_PREDICTIONS_PATH.read_text()))
    generated_at = doc.get("generated_at_utc")
    for p in doc.get("picks") or []:
        p["prop_type"] = p.get("market")
        if p.get("model_prob") is not None:
...
@app.get("/tennis/record")
def tennis_record():
    """Serves the file tennis_grade_record_a.py writes (see
    REPO_TENNIS_RECORD_PATH) -- read-only, no live compute here. Same
    pattern as /nfl/record. Covers both served tennis markets
    (total_games graded against a real line; set_betting graded as an
    unagraded projection's own hit rate -- see that script's docstring)."""
    if not REPO_TENNIS_RECORD_PATH.exists():
        return _EMPTY_RECORD_DOC
    return _sanitize_nan(json.loads(REPO_TENNIS_RECORD_PATH.read_text()))


@app.get("/nhl/record")
def nhl_record():
    """Serves the file nhl_grade_record_a.py writes (see
    REPO_NHL_RECORD_PATH) -- read-only, no live compute here. Same
    pattern as /cfb/record."""
    if not REPO_NHL_RECORD_PATH.exists():
        return _EMPTY_RECORD_DOC
    return _sanitize_nan(json.loads(REPO_NHL_RECORD_PATH.read_text()))

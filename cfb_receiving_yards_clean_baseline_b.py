#!/usr/bin/env python3
"""
CFB_RECEIVING_YARDS_CLEAN_BASELINE_B

gate_a failed receiving_yards decisively (AUC 0.5453, near coin-flip).
Same theory as rushing_yards_clean_baseline_b: college football's huge
team-strength mismatches (Power 4 blowouts of Group of 5, etc.) drive real
receiving-volume effects the original NFL-mirrored feature set never had
to account for -- a blowout can mean garbage-time bench receivers getting
extra late targets, OR a shut-down passing game if a team is run-clock
grinding out a big lead. Adds the same real, non-odds team-strength
context already added for rushing_yards:
  team_net_margin, opp_net_margin, projected_margin (all as-of, strictly
  prior games, built from real final scores already in `games`)

Everything else identical to clean_baseline_a.

Run
---
python -u cfb_receiving_yards_clean_baseline_b.py
"""
...
WORKDIR_DEFAULT = "/data/cfb_model/cfb_receiving_yards_clean_baseline_b_work"
MIN_PRIOR_GAMES_FOR_RATE = 3
MIN_RECENT_RECEPTIONS_PER_GAME = 5
LINE = 59.5  # reuse the line already derived in clean_baseline_a's diagnostic
...
def main():
    ...
    print("CFB_RECEIVING_YARDS_CLEAN_BASELINE_B\n=====================================")
    ...
    manifest = {
        "script": "CFB_RECEIVING_YARDS_CLEAN_BASELINE_B",

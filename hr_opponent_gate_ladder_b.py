#!/usr/bin/env python3
"""
HR_OPPONENT_GATE_LADDER_B

Blunt opposing-pitcher gate before Bayesian pitch-type/zone overlap.

Train: 2025 only
Test:  2026 only

A0 = current ENVIRONMENT_C_LOCKED_A
B1 = A0 + one crude pitcher HR suppression index
B2 = B1 + K/GB/barrel/hard-hit/HR indices
B3 = B2 + pitch mix / whiff / CSW indices

Leak-safe:
  pitch rows must satisfy pitch.game_date < batter_game.game_date
  and pitch.game_date >= DATE(batter_game.game_date, '-365 day')

Run:
  python hr_opponent_gate_ladder_b.py --rebuild-features
"""
...
def require(conn):
    ...
    if "pitcher_id" not in cols(conn,"batter_games"):
        raise SystemExit("Missing batter_games.pitcher_id")
...
def build_gate(conn,rebuild=False):
    ...
    with conn:
        ...
        WITH bgx AS (
          SELECT game_id,batter_id,game_date,pitcher_id FROM batter_games WHERE actual_hr IS NOT NULL
        ),
        pr AS (
...
def expr(conn,alias,cands):
    ...
    for c in cands:
        for alias_tbl,colset in tables:
            if c in colset: return f"{alias_tbl}.{c} AS {alias}"
...
def load(conn,pd):
    ...
    sel=["bg.game_date AS game_date","bg.game_id AS game_id","bg.batter_id AS batter_id","bg.pitcher_id AS pitcher_id","bg.batter_name AS batter_name",opt("team"),opt("opponent"),opt("venue"),"bg.lineup_spot AS lineup_spot","bg.actual_hr AS actual_hr"]
    ...
    LEFT JOIN opponent_gate_features og ON bg.game_id=og.game_id AND bg.batter_id=og.batter_id
...
def main():
    ...
    print("HR_OPPONENT_GATE_LADDER_B")
    print("=========================")
    print("db:",args.db)
    print("building/checking opponent gate features...")
    ...
    print("If they do not improve holdout, debug join/window/framing before building the complex overlap layer.")

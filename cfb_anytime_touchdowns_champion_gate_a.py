#!/usr/bin/env python3
"""
CFB_ANYTIME_TOUCHDOWNS_CHAMPION_GATE_A

The REAL anytime-TD model: rushing_touchdowns and receiving_touchdowns
were each scored separately, which is too narrow -- a real anytime-TD
prop pays out on a score by ANY means. Combines both into one target
(actual_total_touchdowns = rushing_touchdowns + receiving_touchdowns)
for one combined RB+WR population, direct binary:logistic classifier.
Predictions-first: no odds.

Same proven configuration as every other CFB TD market (margin-context
features, regularized hyperparameters, power-adjusted bootstrap
calibration test instead of a fixed ECE bar).

DEV=2018-2022, VAL=2023, HOLDOUT=2024 -- 2025 excluded, same reason as
every other TD market here: cfbfastR's touchdown_player_id field is
measurably less complete for 2025 (confirmed directly in cfb_anytime_
touchdowns_clean_baseline_a.py's own diagnostic run: over_0.5 rate sits
at 0.45-0.47 every season 2018-2024, then falls off a cliff to 0.187 in
2025 -- not a real football trend, a data-completeness gap).
...
Run
---
python -u cfb_anytime_touchdowns_champion_gate_a.py
"""
...
BASELINE_DEFAULT = "/data/cfb_model/cfb_anytime_touchdowns_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "/data/cfb_model/cfb_anytime_touchdowns_champion_gate_a_work"
...
def load(baseline_path):
    ...
    rows = con.execute(f"SELECT {', '.join(cols)} FROM cfb_anytime_touchdowns_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("CFB_ANYTIME_TOUCHDOWNS_CHAMPION_GATE_A\n=======================================")
    ...
    verdict = ("CFB_ANYTIME_TOUCHDOWNS_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "CFB_ANYTIME_TOUCHDOWNS_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    bst.save_model(str(work / "cfb_anytime_touchdowns.json"))
    (work / "cfb_anytime_touchdowns_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "CFB_ANYTIME_TOUCHDOWNS_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
    ...
    (work / "cfb_anytime_touchdowns_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

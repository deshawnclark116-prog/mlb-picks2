#!/usr/bin/env python3
"""
PITCHER_K_D0_IMPLEMENTATION_PARITY_AUDIT_A
...
Run
---
python -u pitcher_k_d0_implementation_parity_a.py 2>&1 | tee /data/hr_model/pitcher_k_d0_implementation_parity_a.log

Outputs
-------
/data/hr_model/pitcher_k_d0_implementation_parity_a_results.json
/data/hr_model/pitcher_k_d0_implementation_parity_a_report.txt
...
OUT_JSON = (
    HR_DIR
    / "pitcher_k_d0_implementation_parity_a_results.json"
)

OUT_TXT = (
    HR_DIR
    / "pitcher_k_d0_implementation_parity_a_report.txt"
)
...
def status_row(
    *,
    ...
    if status not in {"PASS", "BLOCKER", "ADVISORY"}:
...
def audit_d0_activation(api, lineupk):
    ...
    blend_src = lineupk.function_source(
    ...
    general_norm = norm_code(general_src)
    blend_norm = norm_code(blend_src)
    lineup_norm = norm_code(lineup_src)
    run_norm = norm_code(run_src)

    # General source can return real data independently of H2H.
    general_real_data_supported = (
        general_norm.count("returnso/pa,true") >= 2
    )
        ...
        status_row(
            ...
            remediation=(
                ...
                if general_real_data_supported
                    ...
                    "Return a real-data availability flag for "
                    "handedness/season K history."
                )
            ),
        )
    )

    # Critical D0 semantic:
    # production currently returns True only when H2H exists.
    current_true_only_on_h2h = (
        "ifh2h_krisnotnone:"
        in blend_norm
        and "returnH2H_WEIGHT*h2h_kr+GEN_WEIGHT*gen_kr,true"
        in blend_norm
        and "returngen_kr,false"
        in blend_norm
    )

    d0_usable_semantics_present = (
        (
            "returngen_kr,gen_has_data"
            in blend_norm
        )
        or (
            "used=gen_usedor"
    ...
    if d0_usable_semantics_present:
        d0_status = "PASS"
        observed = (
            "usable-data semantics detected"
        )
    elif current_true_only_on_h2h:
        d0_status = "BLOCKER"
        observed = (
            "current flag is True only when H2H exists; "
            "general handedness/season data returns False"
        )
    else:
        d0_status = "BLOCKER"
        observed = (
    ...
    rows.append(
        status_row(
            ...
            observed=observed,
            status=d0_status,
            evidence="lineupk.py::blended_batter_k_rate",
            remediation=(
                None
                if d0_status == "PASS"
                else (
    ...
    rows.append(
        status_row(
            ...
            status="PASS" if api_gate_ok else "BLOCKER",
            evidence="api.py::run_predictions",
            remediation=(
                None
                if api_gate_ok
    ...
    # End-to-end D0 parity requires D0-02 and D0-04.
    end_to_end_ok = (
        d0_status == "PASS"
...
def main():
    ...
    if (
        ...
        final_verdict = (
            "D0_IMPLEMENTATION_PARITY_AUDIT_PASS_"
            "READY_FOR_EXPLICIT_PRODUCTION_PROMOTION_DECISION_"
            "NO_AUTOMATIC_PROMOTION"
        )
    else:
        final_verdict = (
            "D0_IMPLEMENTATION_PARITY_AUDIT_FAIL_"
    ...
    payload = {
        ...
        "policy": {
            ...
            "if_blockers_exist": (
                "patch only validated parity blockers, rerun audit, "
                "then perform live shadow parity check before explicit "
                "promotion decision"
            ),

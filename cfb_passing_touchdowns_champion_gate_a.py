#!/usr/bin/env python3
"""
CFB_PASSING_TOUCHDOWNS_CHAMPION_GATE_A

First honest CFB passing-touchdowns model, direct binary:logistic
classifier. Predictions-first: no odds. QB-only population (see
clean_baseline).

Same proven configuration as passing_yards_champion_gate_a (margin-
context features, regularized hyperparameters, power-adjusted bootstrap
calibration test instead of a fixed ECE bar -- all established/reused,
not re-derived).

DEV=2022, VAL=2023, HOLDOUT=2024 -- NOT the other markets' 2022-2023/
2024/2025 split. cfb_passing_touchdowns_clean_baseline_a.py's docstring
has the full data-quality finding: cfbfastR-data's touchdown_player_id
field is measurably less complete for 2025 specifically (pass-TD rows
drop ~33% year over year despite total plays increasing), so 2025 cannot
be trusted as a holdout for this market. Confirmed by direct inspection,
and the fix (use 2024 as holdout instead) was an explicit user decision,
...
Run
---
python -u cfb_passing_touchdowns_champion_gate_a.py
"""
...
BASELINE_DEFAULT = "/data/cfb_model/cfb_passing_touchdowns_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "/data/cfb_model/cfb_passing_touchdowns_champion_gate_a_work"
...
def load(baseline_path):
    ...
    rows = con.execute(f"SELECT {', '.join(cols)} FROM cfb_passing_touchdowns_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("CFB_PASSING_TOUCHDOWNS_CHAMPION_GATE_A\n=======================================")
    ...
    verdict = ("CFB_PASSING_TOUCHDOWNS_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "CFB_PASSING_TOUCHDOWNS_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    bst.save_model(str(work / "cfb_passing_touchdowns.json"))
    (work / "cfb_passing_touchdowns_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "CFB_PASSING_TOUCHDOWNS_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
    ...
    (work / "cfb_passing_touchdowns_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

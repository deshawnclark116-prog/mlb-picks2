#!/usr/bin/env python3
"""
NHL_PRIOR_SEASON_SHOTS_ON_GOAL_GATE_A
...
zero shots-on-goal OVER/UNDER picks until a skater has 5 real current-season games
(nhl_shots_on_goal_champion_gate_a.py's own eligibility floor), feed each
...
WORKDIR = Path("nhl_models/nhl_prior_season_shots_on_goal_gate_a_work")
MAX_WEEK = 3
...
def main():
    ...
    print("NHL_PRIOR_SEASON_SHOTS_ON_GOAL_GATE_A\n===============================")
    ...
    verdict = f"NHL_PRIOR_SEASON_SHOTS_ON_GOAL_{'PASSES_GATE' if passed else 'DOES_NOT_CLEAR_GATE'}"
    print(f"\n  GATE: AUC>=0.58 -> {c1}   logloss gain>=0.01 -> {c2}   Brier better -> {c3}")
    print(f"  VERDICT: {verdict}")

    bst.save_model(str(WORKDIR / "nhl_prior_season_shots_on_goal.json"))
    (WORKDIR / "nhl_prior_season_shots_on_goal_columns.json").write_text(json.dumps(FEATURE_COLS))
    report = {
        "script": "NHL_PRIOR_SEASON_SHOTS_ON_GOAL_GATE_A",
    ...
    (WORKDIR / "nhl_prior_season_shots_on_goal_gate_a_report.json").write_text(json.dumps(report, indent=2))

#!/usr/bin/env python3
"""
NHL_GOALIE_SAVES_WALKFORWARD_STABILITY_A
...
import nhl_goalie_saves_champion_gate_a as g
...
WORKDIR = Path("nhl_models/nhl_goalie_saves_walkforward_stability_a_work")


def main():
    import xgboost as xgb
    WORKDIR.mkdir(parents=True, exist_ok=True)
    print("NHL_GOALIE_SAVES_WALKFORWARD_STABILITY_A\n" + "=" * 35)
    ...
    verdict = ("NHL_GOALIE_SAVES_WALKFORWARD_STABLE_READY_FOR_LIVE_WIRING" if stable
               else "NHL_GOALIE_SAVES_WALKFORWARD_NOT_YET_STABLE")
    print(f"\n================ FINAL VERDICT ================\n  {verdict}")

    bst.save_model(str(WORKDIR / "nhl_goalie_saves.json"))
    (WORKDIR / "nhl_goalie_saves_columns.json").write_text(json.dumps(g.FEATURES))
    report = {
        "script": "NHL_GOALIE_SAVES_WALKFORWARD_STABILITY_A",
    ...
    (WORKDIR / "nhl_goalie_saves_walkforward_stability_a_report.json").write_text(json.dumps(report, indent=2))

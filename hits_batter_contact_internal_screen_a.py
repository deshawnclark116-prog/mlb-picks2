#!/usr/bin/env python3
"""
HITS_BATTER_CONTACT_INTERNAL_SCREEN_A

Doctrine-safe batch screen for the batter-contact-skill family.
...
- Therefore 2025 is not untouched and cannot be called a clean forward holdout.
- No candidate can earn production promotion from this script alone.

What this script does:
- Uses 2024 only to fit one lightweight correction head per candidate.
...
Candidates
----------
1. batter_contact_rate_60d
2. batter_whiff_rate_60d
3. batter_k_per_pa_60d
...
A candidate must beat:
1. the locked baseline, and
2. the calibration-only control.
...
- SQLite reads only
- small Python lists
- suitable for 512 MB Render instance

Run
---
python -u hits_batter_contact_internal_screen_a.py 2>&1 | tee /data/hr_model/hits_batter_contact_internal_screen_a.log

Outputs
-------
/data/hr_model/hits_batter_contact_internal_screen_a_results.json
/data/hr_model/hits_batter_contact_internal_screen_a_report.txt
...
OUT_JSON = Path("/data/hr_model/hits_batter_contact_internal_screen_a_results.json")
OUT_TXT = Path("/data/hr_model/hits_batter_contact_internal_screen_a_report.txt")
...
def monthly_brier(y, p, dates):
    ...
    for i, date_text in enumerate(dates):
        groups.setdefault(month_key(date_text), []).append(i)
...
def evaluate_candidate(conn, candidate, config, score_2024, score_2025):
    ...
    for month in sorted(baseline_monthly):
        ...
        challenger = candidate_monthly[month]
        delta = challenger["brier"] - base["brier"]
...
def main():
    print("HITS_BATTER_CONTACT_INTERNAL_SCREEN_A", flush=True)
    print("=====================================", flush=True)
    ...
    if not score_2024_path.exists():
        raise RuntimeError(
            f"Missing baseline scores: {score_2024_path}"
        )

    if not score_2025_path.exists():
        raise RuntimeError(
            f"Missing baseline scores: {score_2025_path}"
        )
    ...
    for candidate, result in results.items():
        control_coef = result[
            "calibration_control_fit"
        ]["coefficients"]

        candidate_coef = result[
            "candidate_fit"
        ]["coefficients"]
    ...
    if survivors:
        ...
        print(
            "next_step=FREEZE_THIS_BATTER_CONTACT_SCREEN; "
            "no rescue tuning on 2024/2025.",
            flush=True,
        )

    payload = {
        "script": "HITS_BATTER_CONTACT_INTERNAL_SCREEN_A",

#!/usr/bin/env python3
"""
NHL_SHOTS_ON_GOAL_CHAMPION_GATE_A

Real shots-on-goal OVER/UNDER classifier, same champion-gate process as
...
Run
---
python -u nhl_shots_on_goal_champion_gate_a.py
"""
import argparse
import json
import sqlite3
...
BASELINE_DEFAULT = "nhl_models/nhl_shots_on_goal_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "nhl_models/nhl_shots_on_goal_champion_gate_a_work"
...
def load(baseline_path):
    ...
    rows = con.execute(f"SELECT {', '.join(cols)} FROM nhl_shots_on_goal_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("NHL_SHOTS_ON_GOAL_CHAMPION_GATE_A\n==================================")
    ...
    verdict = ("NHL_SHOTS_ON_GOAL_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "NHL_SHOTS_ON_GOAL_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    bst.save_model(str(work / "nhl_shots_on_goal.json"))
    (work / "nhl_shots_on_goal_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "NHL_SHOTS_ON_GOAL_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
    ...
    (work / "nhl_shots_on_goal_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

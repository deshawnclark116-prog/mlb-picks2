#!/usr/bin/env python3
"""
CFB_FCS_PASSING_YARDS_CHAMPION_GATE_B

gate_a came close: AUC 0.6820 (strongest of any FCS market tried so
far), ECE 0.0257 vs the 0.02 bar -- a much smaller miss than rushing_yards
or receiving_yards saw, concentrated in one holdout bin (0.7-0.8:
pred=0.721 actual=0.866, n=67). Worth a real Platt attempt given how
close this already is, before concluding it needs anything more
invasive.

Platt map fit on the 2024 INTERNAL VAL slice only, never touches the
2025 holdout for fitting.

Run
---
python -u cfb_fcs_passing_yards_champion_gate_b.py
"""
import json
import sys
...
import cfb_fcs_passing_yards_champion_gate_a as g
...
def main():
    ...
    print("CFB_FCS_PASSING_YARDS_CHAMPION_GATE_B\n======================================")
    ...
    verdict = ("CFB_FCS_PASSING_YARDS_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "CFB_FCS_PASSING_YARDS_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    if passed:
        bst.save_model(str(work / "cfb_fcs_passing_yards.json"))
        (work / "cfb_fcs_passing_yards_columns.json").write_text(json.dumps(g.FEATURES))
        (work / "cfb_fcs_passing_yards_platt.json").write_text(json.dumps({"a": a, "b": b}))
    report = {"script": "CFB_FCS_PASSING_YARDS_CHAMPION_GATE_B", "holdout": g.HOLDOUT_SEASON,
    ...
    (work / "cfb_fcs_passing_yards_champion_gate_b_report.json").write_text(json.dumps(report, indent=2))

#!/usr/bin/env python3
"""
NHL_POINTS_WALKFORWARD_STABILITY_A
...
import nhl_points_champion_gate_a as g
...
WORKDIR = Path("nhl_models/nhl_points_walkforward_stability_a_work")


def main():
    import xgboost as xgb
    WORKDIR.mkdir(parents=True, exist_ok=True)
    print("NHL_POINTS_WALKFORWARD_STABILITY_A\n" + "=" * 35)
    ...
    verdict = ("NHL_POINTS_WALKFORWARD_STABLE_READY_FOR_LIVE_WIRING" if stable
               else "NHL_POINTS_WALKFORWARD_NOT_YET_STABLE")
    print(f"\n================ FINAL VERDICT ================\n  {verdict}")

    bst.save_model(str(WORKDIR / "nhl_points.json"))
    (WORKDIR / "nhl_points_columns.json").write_text(json.dumps(g.FEATURES))
    report = {
        "script": "NHL_POINTS_WALKFORWARD_STABILITY_A",
    ...
    (WORKDIR / "nhl_points_walkforward_stability_a_report.json").write_text(json.dumps(report, indent=2))

#!/usr/bin/env python3
"""
CFB_RUSHING_TOUCHDOWNS_CHAMPION_GATE_A

First honest CFB rushing-touchdowns (anytime-TD) model, direct
binary:logistic classifier. Predictions-first: no odds. RB-only
population (see clean_baseline).

Same proven configuration as passing_touchdowns_champion_gate_a
(margin-context features, regularized hyperparameters, power-adjusted
bootstrap calibration test instead of a fixed ECE bar).

DEV=2018-2022, VAL=2023, HOLDOUT=2024 -- 2025 excluded, same reason as
passing_touchdowns: cfbfastR's touchdown_player_id field is measurably
less complete for 2025 (confirmed directly in cfb_rushing_touchdowns_
clean_baseline_a.py's own diagnostic run: over_0.5 rate sits at
0.47-0.52 every season 2018-2024, then falls off a cliff to 0.175 in
...
Run
---
python -u cfb_rushing_touchdowns_champion_gate_a.py
"""
...
BASELINE_DEFAULT = "/data/cfb_model/cfb_rushing_touchdowns_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "/data/cfb_model/cfb_rushing_touchdowns_champion_gate_a_work"
...
def load(baseline_path):
    ...
    rows = con.execute(f"SELECT {', '.join(cols)} FROM cfb_rushing_touchdowns_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("CFB_RUSHING_TOUCHDOWNS_CHAMPION_GATE_A\n=======================================")
    ...
    verdict = ("CFB_RUSHING_TOUCHDOWNS_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "CFB_RUSHING_TOUCHDOWNS_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    bst.save_model(str(work / "cfb_rushing_touchdowns.json"))
    (work / "cfb_rushing_touchdowns_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "CFB_RUSHING_TOUCHDOWNS_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
    ...
    (work / "cfb_rushing_touchdowns_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

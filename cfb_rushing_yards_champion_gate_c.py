#!/usr/bin/env python3
"""
CFB_RUSHING_YARDS_CHAMPION_GATE_C

gate_a passed AUC/logloss/Brier but failed ECE (0.0410 vs 0.02 bar) --
the reliability table showed a real gap concentrated around the 0.6-0.7
bin (pred=0.643 vs actual=0.485), plus a mild base-rate softening in 2025
vs the dev seasons (over-rate 51.3%/49.4% dev -> 44.3% 2025 holdout).
Unlike the NFL tackles market's continuous multi-year drift (which Platt
scaling failed to fix), this looks like a single, more standard
calibration gap -- worth a real Platt-scaling attempt, same technique that
worked cleanly for NFL sacks.
...
Run
---
python -u cfb_rushing_yards_champion_gate_b.py
"""
import json
import sys
...
import cfb_rushing_yards_champion_gate_b2 as g
...
def main():
    ...
    print("CFB_RUSHING_YARDS_CHAMPION_GATE_C\n==================================")
    ...
    (work / "cfb_rushing_yards_platt.json").write_text(json.dumps({"a": a, "b": b}))
    report = {"script": "CFB_RUSHING_YARDS_CHAMPION_GATE_C", "holdout": g.HOLDOUT_SEASON,
    ...
    (work / "cfb_rushing_yards_champion_gate_b_report.json").write_text(json.dumps(report, indent=2))

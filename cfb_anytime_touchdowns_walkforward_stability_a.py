#!/usr/bin/env python3
"""
CFB_ANYTIME_TOUCHDOWNS_WALKFORWARD_STABILITY_A
...
Uses the exact model/features/hyperparameters cfb_anytime_touchdowns_
...
data-quality finding in cfb_anytime_touchdowns_clean_baseline_a.py
...
Run
---
python -u cfb_anytime_touchdowns_walkforward_stability_a.py
"""
import json
import sys
...
import cfb_anytime_touchdowns_champion_gate_a as g
...
WORKDIR = Path("/data/cfb_model/cfb_anytime_touchdowns_walkforward_stability_a_work")
...
def main():
    ...
    print("CFB_ANYTIME_TOUCHDOWNS_WALKFORWARD_STABILITY_A\n" + "=" * 42)
    ...
    verdict = ("CFB_ANYTIME_TOUCHDOWNS_WALKFORWARD_STABLE_READY_FOR_LIVE_WIRING" if stable
               else "CFB_ANYTIME_TOUCHDOWNS_WALKFORWARD_NOT_YET_STABLE")
    print(f"\n================ FINAL VERDICT ================\n  {verdict}")

    bst.save_model(str(WORKDIR / "cfb_anytime_touchdowns.json"))
    (WORKDIR / "cfb_anytime_touchdowns_columns.json").write_text(json.dumps(g.FEATURES))
    report = {
        "script": "CFB_ANYTIME_TOUCHDOWNS_WALKFORWARD_STABILITY_A",
    ...
    (WORKDIR / "cfb_anytime_touchdowns_walkforward_stability_a_report.json").write_text(json.dumps(report, indent=2))

#!/usr/bin/env python3
"""
CFB_RECEIVING_TOUCHDOWNS_CLEAN_BASELINE_B

clean_baseline_a (all-division population) fell short: AUC 0.5732 (bar
0.58), logloss gain +0.00852 (bar 0.01) -- real but too weak, on the 2024
holdout. Same fix already validated for receiving_yards, tested here
rather than assumed to transfer: restrict to Power4-vs-Power4 games only
(cfb_receiving_yards_clean_baseline_c/d.py's rationale: WR usage in
Group-of-5/mismatch games was the noisiest slice and suppressed AUC
there too).

Anytime-receiving-TD prop for WRs. Same population/eligibility/margin-
context design as receiving_yards -- only the target changes (did this
WR score >=1 receiving TD this game, not a yardage line).

LINE=0.5: diagnostic-derived from real 2018-2024 data (2025 excluded --
same touchdown_player_id completeness caveat as every other TD market
here, re-verified for receiving specifically, not assumed to carry over).
Among eligible WR games (>=3 prior games, recent3 receptions >=5),
receiving TD distribution was 0:2623, 1:1186, 2:325, 3:51, 4:9, 5:1 --
over 0.5 (>=1 TD) splits 37.5%/62.5%.

Built on the CORRECTED player_games (completion/reception swap bug fixed
in cfb_player_games_foundation_a.py) -- receiving_touchdowns is exactly
the stat that bug corrupted most directly (a real WR's touchdown catch
was landing on the QB's row before the fix), so this market only exists
because that fix landed first.

Run
---
python -u cfb_receiving_touchdowns_clean_baseline_b.py
"""
...
WORKDIR_DEFAULT = "/data/cfb_model/cfb_receiving_touchdowns_clean_baseline_b_work"
...
def build_rows(conn):
    ...
    rows = conn.execute("""
        SELECT player_id, player_name, team, opponent, season, week,
               is_home, receptions, receiving_touchdowns, game_id
        FROM player_games
        WHERE position = 'WR'
    ...
    def flush(group):
        ...
        for r in group:
            pid, pname, team, opp, season, week, is_home, receptions, rec_td, gid = r
            c3 = list(rec_hist)[-3:]
            ...
            if (n_prior >= MIN_PRIOR_GAMES_FOR_RATE
                ...
                feat = {
                    "season_avg_rec_td": cum_td / n_prior,
                    "recent3_avg_rec_td": sum(r3) / len(r3) if r3 else 0.0,
                    "recent5_avg_rec_td": sum(r5) / len(r5) if r5 else 0.0,
                    "season_avg_receptions": cum_rec / n_prior,
                    "recent3_avg_receptions": sum(c3) / len(c3) if c3 else 0.0,
                    "td_per_reception": (cum_td / cum_rec) if cum_rec > 0 else 0.0,
                    "opp_rec_td_allowed_per_game": opp_asof.get((pid, season, week)),
                ...
                actual_td = rec_td if rec_td is not None else 0
                out.append({
                    "player_id": pid, "player_name": pname, "team": team,
                    "opponent": opp, "season": season, "week": week,
                    **feat, "actual_receiving_touchdowns": actual_td,
                })
            cum_td += rec_td if rec_td is not None else 0
            cum_rec += receptions if receptions is not None else 0
            td_hist.append(rec_td if rec_td is not None else 0)
...
def main():
    ...
    print("CFB_RECEIVING_TOUCHDOWNS_CLEAN_BASELINE_B\n==========================================")
    print(f"source={src}\nworkdir={work}\nline={LINE}")
    ...
    out.execute(f"""CREATE TABLE cfb_receiving_touchdowns_baseline (
        player_id TEXT, player_name TEXT, team TEXT, opponent TEXT,
        season INTEGER, week INTEGER, {cols_sql},
        actual_receiving_touchdowns INTEGER, over_line INTEGER
    )""")
    insert_cols = (["player_id", "player_name", "team", "opponent", "season", "week"]
                   + MODEL_COLUMNS + ["actual_receiving_touchdowns", "over_line"])
    placeholder = ", ".join("?" for _ in insert_cols)
    ins = f"INSERT INTO cfb_receiving_touchdowns_baseline ({', '.join(insert_cols)}) VALUES ({placeholder})"

    by_season = {}
    batch = []
    for r in rows:
        r["over_line"] = 1 if r["actual_receiving_touchdowns"] >= (LINE + 0.5) else 0
    ...
    manifest = {
        "script": "CFB_RECEIVING_TOUCHDOWNS_CLEAN_BASELINE_B",
        ...
        "target": f"over_line = actual_receiving_touchdowns >= {LINE + 0.5} (line {LINE})",
        "line": LINE,

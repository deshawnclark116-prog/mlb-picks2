#!/usr/bin/env python3
"""
CFB_RECEIVING_YARDS_WALKFORWARD_STABILITY_A
...
Run
---
python -u cfb_receiving_yards_walkforward_stability_a.py
"""
import json
import sys
...
WORKDIR = Path("/data/cfb_model/cfb_receiving_yards_walkforward_stability_a_work")
...
def main():
    ...
    print("CFB_RECEIVING_YARDS_WALKFORWARD_STABILITY_A\n" + "=" * 44)
    ...
    verdict = ("CFB_RECEIVING_YARDS_WALKFORWARD_STABLE_READY_FOR_LIVE_WIRING" if stable
               else "CFB_RECEIVING_YARDS_WALKFORWARD_NOT_YET_STABLE")
    print(f"\n================ FINAL VERDICT ================\n  {verdict}")

    bst.save_model(str(WORKDIR / "cfb_receiving_yards.json"))
    (WORKDIR / "cfb_receiving_yards_columns.json").write_text(json.dumps(g.FEATURES))
    report = {
        "script": "CFB_RECEIVING_YARDS_WALKFORWARD_STABILITY_A",
    ...
    (WORKDIR / "cfb_receiving_yards_walkforward_stability_a_report.json").write_text(json.dumps(report, indent=2))

#!/usr/bin/env python3
"""
NFL_ANYTIME_TOUCHDOWNS_WALKFORWARD_STABILITY_A

Final rung before live wiring, mirroring cfb_anytime_touchdowns_walkforward_
stability_a.py exactly (same two-rung structure, same bars). Uses the exact
model/features/hyperparameters nfl_anytime_touchdowns_champion_gate_a already
validated -- this script does not retrain a different model, it re-examines
the already-passing predictions for consistency.

DEV=2020-2023, VAL=2024, HOLDOUT=2025 -- same split as the champion gate.
...
calibration_policy would behave.

RUNG 2 -- stability confirmation (same bars as CFB's TD markets): week-block
bootstrap AUC 95% CI lower >= 0.55; season quarters AUC >= 0.50 in >= 3/4
quarters, none < 0.45; home/away slices (min 100 rows) AUC >= 0.52 and
calibration p >= 0.10.

Read-only. Writes only a report + the frozen model this repo's serving
builder actually loads.

Run
---
python -u nfl_anytime_touchdowns_walkforward_stability_a.py
"""
import json
import sys
...
import nfl_anytime_touchdowns_champion_gate_a as g
...
WORKDIR = Path("nfl_models/nfl_anytime_touchdowns_walkforward_stability_a_work")
...
def main():
    ...
    print("NFL_ANYTIME_TOUCHDOWNS_WALKFORWARD_STABILITY_A\n" + "=" * 42)
    ...
    verdict = ("NFL_ANYTIME_TOUCHDOWNS_WALKFORWARD_STABLE_READY_FOR_LIVE_WIRING" if stable
               else "NFL_ANYTIME_TOUCHDOWNS_WALKFORWARD_NOT_YET_STABLE")
    print(f"\n================ FINAL VERDICT ================\n  {verdict}")

    bst.save_model(str(WORKDIR / "nfl_anytime_touchdowns.json"))
    (WORKDIR / "nfl_anytime_touchdowns_columns.json").write_text(json.dumps(g.FEATURES))
    report = {
        "script": "NFL_ANYTIME_TOUCHDOWNS_WALKFORWARD_STABILITY_A",
    ...
    (WORKDIR / "nfl_anytime_touchdowns_walkforward_stability_a_report.json").write_text(json.dumps(report, indent=2))

#!/usr/bin/env python3
"""
CFB_MONEYLINE_WALKFORWARD_STABILITY_A

Final rung before live wiring, mirroring cfb_anytime_touchdowns_
walkforward_stability_a.py exactly (same two-rung structure, same bars).
Uses the exact model/features/hyperparameters cfb_moneyline_champion_
gate_a already validated -- this script does not retrain a different
model, it re-examines the already-passing predictions for consistency.

DEV=2018-2022, VAL=2023, HOLDOUT=2024 -- same split as the champion gate.
...
calibration_policy would behave. Fit SEPARATELY for home and away rows
(two independent Platt maps per week-step, not one pooled map applied to
both) -- a first pass with one pooled map passed everything except the
away slice's own calibration test (p=0.077 vs bar 0.10, AUC unaffected at
0.752 either way, so it's a calibration-mapping issue, not a
discrimination one). Real home-field effects plausibly don't shift the
raw classifier's score-to-outcome mapping identically for the home vs
away perspective row of the same game; splitting the fit fixed it
cleanly (away calib_p 0.077 -> 0.389) without touching either pass bar.
...
Run
---
python -u cfb_moneyline_walkforward_stability_a.py
"""
import json
import sys
...
import cfb_moneyline_champion_gate_a as g
...
WORKDIR = Path("cfb_models/cfb_moneyline_walkforward_stability_a_work")
...
def main():
    ...
    print("CFB_MONEYLINE_WALKFORWARD_STABILITY_A\n" + "=" * 38)
    ...
    # ---- RUNG 1: weekly walk-forward Platt, fit separately per home/away ----
    ...
    for name, mask in (("home", hol_home), ("away", ~hol_home)):
        idx = np.where(mask)[0]
    ...
    verdict = ("CFB_MONEYLINE_WALKFORWARD_STABLE_READY_FOR_LIVE_WIRING" if stable
               else "CFB_MONEYLINE_WALKFORWARD_NOT_YET_STABLE")
    print(f"\n================ FINAL VERDICT ================\n  {verdict}")

    bst.save_model(str(WORKDIR / "cfb_moneyline.json"))
    (WORKDIR / "cfb_moneyline_columns.json").write_text(json.dumps(g.FEATURES))
    report = {
        "script": "CFB_MONEYLINE_WALKFORWARD_STABILITY_A",
    ...
    (WORKDIR / "cfb_moneyline_walkforward_stability_a_report.json").write_text(json.dumps(report, indent=2))

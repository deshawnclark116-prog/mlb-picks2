#!/usr/bin/env python3
"""
CFB_RECEIVING_YARDS_CHAMPION_GATE_D

Same model/features/hyperparameters/gate as gate_c -- the ONLY change is
the expanded training pool from clean_baseline_d (2018-2023 dev seasons
instead of 2022-2023, using cfbfastR-data history pulled back further).
VAL_SEASON (2024) and HOLDOUT_SEASON (2025) are unchanged from gate_c, so
this is an honest apples-to-apples comparison of "does more training data
resolve the AUC-CI-straddles-the-bar inconclusive result from gate_c".

Pre-registered pass bar (identical to gate_c, not loosened):
  1. AUC >= 0.58
...
Run
---
python -u cfb_receiving_yards_champion_gate_d.py
"""
import json
import sqlite3
import sys
...
WORKDIR_DEFAULT = "/data/cfb_model/cfb_receiving_yards_champion_gate_d_work"
...
def main():
    ...
    print("CFB_RECEIVING_YARDS_CHAMPION_GATE_D\n====================================")
    ...
    report = {"script": "CFB_RECEIVING_YARDS_CHAMPION_GATE_D", "holdout": HOLDOUT_SEASON,
    ...
    (work / "cfb_receiving_yards_champion_gate_d_report.json").write_text(json.dumps(report, indent=2))

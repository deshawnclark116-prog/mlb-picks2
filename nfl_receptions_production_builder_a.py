...
Run once per line (--line 1.5 or --line 2.5). Retrains the frozen 12-feature
spec on ALL data (2023+2024, last 10% of weeks held out internally for early
stopping) and exports:
    nfl_receptions_over_1_5.json / nfl_receptions_over_2_5.json
    nfl_receptions_over_1_5_columns.json / nfl_receptions_over_2_5_columns.json

Deploying to /data/nfl_models is a separate, explicit step (same discipline as
every MLB model: nothing ships without a human copying the file -- both the
model AND its columns file, together).

Read-only on the clean baseline. Writes only its own work dir.

Run (Render)
------------
python -u nfl_receptions_production_builder_a.py --line 1.5 2>&1 | tee /data/nfl_model/nfl_receptions_production_builder_a_1_5.log
python -u nfl_receptions_production_builder_a.py --line 2.5 2>&1 | tee /data/nfl_model/nfl_receptions_production_builder_a_2_5.log
"""
...
def main():
    ...
    print(f"NFL_RECEPTIONS_PRODUCTION_BUILDER_A  [line={args.line}]\n" + "=" * 40)

    con = sqlite3.connect(f"file:{args.baseline}?mode=ro", uri=True)
    cols = ["season", "week"] + g.FEATURES + [target_col]
    ...
    (work / f"{model_stem}_columns.json").write_text(json.dumps(g.FEATURES))
    ...
    print(f"   {work / (model_stem + '.json')}")
    print(f"   {work / (model_stem + '_columns.json')}")

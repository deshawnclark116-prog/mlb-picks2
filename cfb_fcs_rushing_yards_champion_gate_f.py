#!/usr/bin/env python3
"""
CFB_FCS_RUSHING_YARDS_CHAMPION_GATE_F

Runs against clean_baseline_d: gate_e's scholarship-conference scoping
(excludes only Ivy/Pioneer, 1610 of 2095 games remain) barely moved
calibration (0.0437 -> 0.0422). Narrower cut here, same principle FBS's
own Power4 scoping actually used (a small, competitively homogeneous top
tier, not "everyone except the worst two"): Missouri Valley (MVFC), Big
Sky, and CAA -- the three FCS conferences widely recognized outside this
project as the sport's competitive top tier, not derived from this
project's own performance numbers. 452 of 2095 games (21.6%, almost
exactly FBS's own Power4-vs-Power4 fraction).
...
Run
---
python -u cfb_fcs_rushing_yards_champion_gate_f.py
"""
...
BASELINE_DEFAULT = "/data/cfb_model/cfb_fcs_rushing_yards_clean_baseline_d_work/baseline.sqlite"
WORKDIR_DEFAULT = "/data/cfb_model/cfb_fcs_rushing_yards_champion_gate_f_work"
...
def main():
    ...
    print("CFB_FCS_RUSHING_YARDS_CHAMPION_GATE_F\n======================================")
    ...
    report = {"script": "CFB_FCS_RUSHING_YARDS_CHAMPION_GATE_F", "holdout": HOLDOUT_SEASON,
              "constant": constant, "challenger": challenger, "gate": GATE,
              "passed": passed, "verdict": verdict, "importance": imp,
              "best_iteration": bst.best_iteration}
    (work / "cfb_fcs_rushing_yards_champion_gate_f_report.json").write_text(json.dumps(report, indent=2))

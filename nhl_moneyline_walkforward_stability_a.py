#!/usr/bin/env python3
"""
NHL_MONEYLINE_WALKFORWARD_STABILITY_A

Final rung before live wiring, mirroring cfb_moneyline_walkforward_
stability_a.py exactly (same two-rung structure, same bars, same fix of
fitting SEPARATE home/away Platt maps rather than one pooled map --
applied here from the start rather than discovered by a first failing
attempt, since it's already known to matter for this exact model shape).

DEV=2018-2022, VAL=2023, HOLDOUT=2024 -- same split as the champion gate.

RUNG 1 -- walk-forward calibration: as each holdout-season week finishes,
refit a 2-parameter Platt map (fit separately for home/away rows) on the
internal-val slice + holdout weeks seen so far, predict the NEXT week
with it.

RUNG 2 -- stability confirmation (same bars as every other market):
week-block bootstrap AUC 95% CI lower >= 0.55; season quarters AUC >=
0.50 in >= 3/4 quarters, none < 0.45; home/away slices (min 100 rows)
AUC >= 0.52 and calibration p >= 0.10.

Read-only. Writes only a report + model artifact.

Run
---
python -u nhl_moneyline_walkforward_stability_a.py
"""
import json
import sys
...
import nhl_moneyline_champion_gate_a as g
...
WORKDIR = Path("nhl_models/nhl_moneyline_walkforward_stability_a_work")
...
def main():
    ...
    print("NHL_MONEYLINE_WALKFORWARD_STABILITY_A\n" + "=" * 38)
    ...
    verdict = ("NHL_MONEYLINE_WALKFORWARD_STABLE_READY_FOR_LIVE_WIRING" if stable
               else "NHL_MONEYLINE_WALKFORWARD_NOT_YET_STABLE")
    print(f"\n================ FINAL VERDICT ================\n  {verdict}")

    bst.save_model(str(WORKDIR / "nhl_moneyline.json"))
    (WORKDIR / "nhl_moneyline_columns.json").write_text(json.dumps(g.FEATURES))
    report = {
        "script": "NHL_MONEYLINE_WALKFORWARD_STABILITY_A",
    ...
    (WORKDIR / "nhl_moneyline_walkforward_stability_a_report.json").write_text(json.dumps(report, indent=2))

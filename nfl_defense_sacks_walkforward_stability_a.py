#!/usr/bin/env python3
"""
NFL_DEFENSE_SACKS_WALKFORWARD_STABILITY_A

sacks passed the static dev/holdout champion gate cleanly (AUC 0.6394, ECE
0.0117, no Platt needed -- unlike tackles, the base rate showed no
season-over-season drift in the clean baseline). Before wiring into live
serving, this repo's house standard for a season-long market requires two
more rungs (rushing_yards needed both, and failed calibration hard the
first time through them -- 2023->2024 base rate shifted 54.67%->65.71%,
exactly the kind of thing a single static split can hide). Consolidated
into one script here (both rungs, not skipped) since sacks' base rate
diagnostic already ruled out the specific failure mode (drift) the split
rushing_yards scripts were built to chase down:

RUNG 1 -- walk-forward calibration (deployment-shaped test): as each 2025
week finishes, refit a 2-parameter Platt map on the 2024 warmup slice +
2025 weeks seen so far (strictly earlier only, no peeking), predict the
NEXT week with it. This is exactly how nfl_serving_builder_a.py's
'growing' calibration_policy behaves live -- reproduced here for
validation, not assumed to work just because the static split passed.
Gate: AUC >= 0.58, logloss gain >= 0.01, Brier beats constant, calibration
bootstrap goodness-of-fit p >= 0.10 on the pooled walk-forward predictions.

RUNG 2 -- stability confirmation (same bars as
nfl_walkforward_stability_confirmation_a.py, reused verbatim since these
markets are comparable in size/AUC): week-block bootstrap AUC 95% CI lower
>= 0.55; season quarters AUC >= 0.50 in >= 3/4 quarters, none < 0.45;
home/away slices (min 100 rows) AUC >= 0.52 and calibration p >= 0.10.

The underlying champion model is FROZEN throughout -- only the Platt map
updates weekly. Read-only on the champion-gate model + baseline. Writes
only a report.

Run
---
python -u nfl_defense_sacks_walkforward_stability_a.py
"""
...
import nfl_defense_sacks_champion_gate_a as g
...
def main():
    ...
    mdir = Path(g.WORKDIR_DEFAULT)
    print("NFL_DEFENSE_SACKS_WALKFORWARD_STABILITY_A\n" + "=" * 42)

    bst = xgb.Booster(); bst.load_model(str(mdir / "nfl_defense_sacks.json"))
    feat_cols = json.loads((mdir / "nfl_defense_sacks_columns.json").read_text())
    assert feat_cols == g.FEATURES
    itr = (0, bst.best_iteration + 1)
    print(f"scoring with iteration_range={itr}")

    dev, hol = g.load(g.BASELINE_DEFAULT)
    cut = g.pick_val_cut(dev)
    tr_dummy = [r for r in dev if (r[0], r[1]) < cut]  # unused, mirrors gate split
    va = [r for r in dev if (r[0], r[1]) >= cut]
    print(f"warmup calibration pool: dev weeks >= {cut}, n={len(va)}")
    ...
    print(f"2025 holdout: n={len(hol_y)} across weeks {weeks[0]}..{weeks[-1]}")
    ...
    print(f"\n============ RUNG 1: WALK-FORWARD CALIBRATION (2025) ============")
    ...
    # ---- RUNG 2: stability confirmation (same bars as receiving/rushing) ----
    ...
    verdict = ("NFL_DEFENSE_SACKS_WALKFORWARD_STABLE_READY_FOR_LIVE_WIRING" if stable
               else "NFL_DEFENSE_SACKS_WALKFORWARD_NOT_YET_STABLE")
    print(f"\n================ FINAL VERDICT ================\n  {verdict}")

    report = {
        "script": "NFL_DEFENSE_SACKS_WALKFORWARD_STABILITY_A",
    ...
    report_path = mdir / "nfl_defense_sacks_walkforward_stability_a_report.json"

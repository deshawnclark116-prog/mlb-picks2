#!/usr/bin/env python3
"""
CFB_FCS_RECEIVING_YARDS_CLEAN_BASELINE_A

FCS analog of the FBS receiving_yards baseline (WR population, same
margin-feature design). Same DEV/VAL/HOLDOUT structure as the FCS
rushing_yards build (DEV=2022-2023, VAL=2024, HOLDOUT=2025) now that the
corrected FCS backfill (see cfb_espn_fcs_historical_foundation_a.py) has
comparable real volume in every season. MIN_RECENT_RECEPTIONS_PER_GAME
kept identical to FBS's own value (5) -- no data-driven reason to loosen
it now that the roster-lookup bug is fixed. LINE derived fresh from real
FCS data, not copied from FBS's 59.5.

Run
---
python -u cfb_fcs_receiving_yards_clean_baseline_a.py
"""
...
WORKDIR_DEFAULT = "/data/cfb_model/cfb_fcs_receiving_yards_clean_baseline_a_work"
MIN_PRIOR_GAMES_FOR_RATE = 3
MIN_RECENT_RECEPTIONS_PER_GAME = 5
THRESHOLD_CANDIDATES = [19.5, 29.5, 39.5, 49.5, 59.5, 69.5]
...
def main():
    ...
    print("CFB_FCS_RECEIVING_YARDS_CLEAN_BASELINE_A\n=========================================")
    print(f"source={src}\nworkdir={work}")
    ...
    out.execute(f"""CREATE TABLE cfb_fcs_receiving_yards_baseline (
    ...
    ins = f"INSERT INTO cfb_fcs_receiving_yards_baseline ({', '.join(insert_cols)}) VALUES ({placeholder})"
    ...
    manifest = {
        "script": "CFB_FCS_RECEIVING_YARDS_CLEAN_BASELINE_A",

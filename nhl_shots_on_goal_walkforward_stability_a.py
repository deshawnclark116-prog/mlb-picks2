#!/usr/bin/env python3
"""
NHL_SHOTS_ON_GOAL_WALKFORWARD_STABILITY_A

Final rung before live wiring, mirroring cfb_anytime_touchdowns_
walkforward_stability_a.py exactly: single pooled Platt map per week
(not split home/away -- this is a player-level market, not moneyline's
symmetric two-rows-per-game team construction, so there's no home/away
mapping-asymmetry reason to split it).

RUNG 1 -- weekly walk-forward Platt calibration on the 2024 holdout,
warmed up on the 2023 internal-val slice.
RUNG 2 -- week-block bootstrap AUC 95% CI lower >= 0.55; season quarters
AUC >= 0.50 in >= 3/4 quarters, none < 0.45; home/away slices (min 100
rows) AUC >= 0.52 and calibration p >= 0.10.

Read-only. Writes only a report + model artifact.

Run
---
python -u nhl_points_walkforward_stability_a.py
"""
import json
import sys
...
import nhl_shots_on_goal_champion_gate_a as g
...
WORKDIR = Path("nhl_models/nhl_shots_on_goal_walkforward_stability_a_work")


def main():
    import xgboost as xgb
    WORKDIR.mkdir(parents=True, exist_ok=True)
    print("NHL_SHOTS_ON_GOAL_WALKFORWARD_STABILITY_A\n" + "=" * 35)
    ...
    observed_ece, calib_p = g.bootstrap_calib_p(wf_pred, hol_y, rng)
    ...
    for name, mask in (("home", home_mask), ("away", ~home_mask)):
        ...
        a = g.auc(probs[idx], y[idx])
        s_ece, s_p = g.bootstrap_calib_p(probs[idx], y[idx], rng)
    ...
    verdict = ("NHL_SHOTS_ON_GOAL_WALKFORWARD_STABLE_READY_FOR_LIVE_WIRING" if stable
               else "NHL_SHOTS_ON_GOAL_WALKFORWARD_NOT_YET_STABLE")
    print(f"\n================ FINAL VERDICT ================\n  {verdict}")

    bst.save_model(str(WORKDIR / "nhl_shots_on_goal.json"))
    (WORKDIR / "nhl_shots_on_goal_columns.json").write_text(json.dumps(g.FEATURES))
    report = {
        "script": "NHL_SHOTS_ON_GOAL_WALKFORWARD_STABILITY_A",
    ...
    (WORKDIR / "nhl_shots_on_goal_walkforward_stability_a_report.json").write_text(json.dumps(report, indent=2))

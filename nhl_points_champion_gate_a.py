#!/usr/bin/env python3
"""
NHL_POINTS_CHAMPION_GATE_A

Real "anytime point" classifier, mirroring the same proven champion-gate
process every other market in this repo goes through: regularized
XGBoost binary:logistic, DEV/VAL/HOLDOUT season splits, power-adjusted
bootstrap calibration test, pre-registered 4-bar pass gate. auc/metrics/
bootstrap_calib_p are imported from nhl_moneyline_champion_gate_a.py
rather than redefined -- identical math, no reason to triple it across
three new player-market gate scripts.

DEV=2018-2022, VAL=2023, HOLDOUT=2024 -- same split convention as every
other market in this repo.

Pre-registered pass bar (all): AUC >= 0.58, calib_p >= 0.10, logloss
gain >= 0.01, Brier better than constant.

Run
---
python -u nhl_points_champion_gate_a.py
"""
import argparse
import json
import sqlite3
...
BASELINE_DEFAULT = "nhl_models/nhl_points_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "nhl_models/nhl_points_champion_gate_a_work"
...
def load(baseline_path):
    ...
    rows = con.execute(f"SELECT {', '.join(cols)} FROM nhl_points_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("NHL_POINTS_CHAMPION_GATE_A\n===========================")
    ...
    verdict = ("NHL_POINTS_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "NHL_POINTS_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    bst.save_model(str(work / "nhl_points.json"))
    (work / "nhl_points_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "NHL_POINTS_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
    ...
    (work / "nhl_points_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

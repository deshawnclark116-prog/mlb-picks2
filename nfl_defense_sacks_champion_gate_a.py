#!/usr/bin/env python3
"""
NFL_DEFENSE_SACKS_CHAMPION_GATE_A

Third individual-defensive-player-prop gate (tackles failed on calibration
drift; this market's base rate was verified stable across 2022-2025 in the
clean baseline before building this). Predictions-first: no odds.
...
CRITICAL: every predict() call after reloading a saved model MUST pass
...
Run (Render)
------------
python -u nfl_defense_sacks_champion_gate_a.py 2>&1 | tee /data/nfl_model/nfl_defense_sacks_champion_gate_a.log
"""
...
BASELINE_DEFAULT = "/data/nfl_model/nfl_defense_sacks_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "/data/nfl_model/nfl_defense_sacks_champion_gate_a_work"
...
def load(baseline_path):
    ...
    rows = con.execute(f"SELECT {', '.join(cols)} FROM nfl_defense_sacks_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("NFL_DEFENSE_SACKS_CHAMPION_GATE_A\n==================================")
    ...
    verdict = ("NFL_DEFENSE_SACKS_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "NFL_DEFENSE_SACKS_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    bst.save_model(str(work / "nfl_defense_sacks.json"))
    (work / "nfl_defense_sacks_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "NFL_DEFENSE_SACKS_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
              "constant": constant, "challenger": challenger, "gate": GATE,
              "passed": passed, "verdict": verdict, "importance": imp,
              "best_iteration": bst.best_iteration}
    (work / "nfl_defense_sacks_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

#!/usr/bin/env python3
"""
CFB_RECEIVING_TOUCHDOWNS_CLEAN_BASELINE_A
...
WR score >=1 receiving TD this game, not a yardage line). Starts at the
ALL-DIVISION population (no Power4 scoping) -- receiving_yards needed
that scoping because WR usage in Group-of-5/mismatch games suppressed
the YARDS signal specifically; whether the same is true for TD-scoring
rate is a separate, untested question, not assumed to transfer.
...
Run
---
python -u cfb_receiving_touchdowns_clean_baseline_a.py
"""
...
WORKDIR_DEFAULT = "/data/cfb_model/cfb_receiving_touchdowns_clean_baseline_a_work"
...
def build_rows(conn):
    ...
    rows = conn.execute("""
        SELECT player_id, player_name, team, opponent, season, week,
               is_home, receptions, receiving_touchdowns
        FROM player_games
        WHERE position = 'WR'
    ...
    def flush(group):
        ...
        for r in group:
            pid, pname, team, opp, season, week, is_home, receptions, rec_td = r
            c3 = list(rec_hist)[-3:]
...
def main():
    ...
    print("CFB_RECEIVING_TOUCHDOWNS_CLEAN_BASELINE_A\n==========================================")
    print(f"source={src}\nworkdir={work}\nline={LINE}")
    ...
    manifest = {
        "script": "CFB_RECEIVING_TOUCHDOWNS_CLEAN_BASELINE_A",

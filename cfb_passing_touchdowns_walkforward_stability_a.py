#!/usr/bin/env python3
"""
CFB_PASSING_TOUCHDOWNS_WALKFORWARD_STABILITY_A
...
exact model/features/hyperparameters cfb_passing_touchdowns_champion_
...
DEV=2022, VAL=2023, HOLDOUT=2024 -- not 2025, per the documented data-
quality finding in cfb_passing_touchdowns_clean_baseline_a.py (cfbfastR-
data's touchdown_player_id field is measurably incomplete for 2025).
...
Run
---
python -u cfb_passing_touchdowns_walkforward_stability_a.py
"""
import json
import sys
...
import cfb_passing_touchdowns_champion_gate_a as g
...
WORKDIR = Path("/data/cfb_model/cfb_passing_touchdowns_walkforward_stability_a_work")
...
def main():
    ...
    print("CFB_PASSING_TOUCHDOWNS_WALKFORWARD_STABILITY_A\n" + "=" * 42)
    ...
    verdict = ("CFB_PASSING_TOUCHDOWNS_WALKFORWARD_STABLE_READY_FOR_LIVE_WIRING" if stable
               else "CFB_PASSING_TOUCHDOWNS_WALKFORWARD_NOT_YET_STABLE")
    print(f"\n================ FINAL VERDICT ================\n  {verdict}")

    bst.save_model(str(WORKDIR / "cfb_passing_touchdowns.json"))
    (WORKDIR / "cfb_passing_touchdowns_columns.json").write_text(json.dumps(g.FEATURES))
    report = {
        "script": "CFB_PASSING_TOUCHDOWNS_WALKFORWARD_STABILITY_A",
    ...
    (WORKDIR / "cfb_passing_touchdowns_walkforward_stability_a_report.json").write_text(json.dumps(report, indent=2))

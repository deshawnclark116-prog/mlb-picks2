#!/usr/bin/env python3
"""
NFL_ANYTIME_TOUCHDOWNS_CHAMPION_GATE_A

The REAL anytime-TD model for NFL, mirroring cfb_anytime_touchdowns_champion_
gate_a.py exactly: rushing_tds and receiving_tds combined into one target
(actual_total_touchdowns), one combined RB+WR population, direct binary:
logistic classifier. Predictions-first: no odds here (real book odds get
attached downstream in serving, same as rushing_yards/receiving_yards).

Same proven configuration as CFB's TD markets (margin-context features,
regularized hyperparameters, power-adjusted bootstrap calibration test
instead of a fixed ECE bar).

DEV=2020-2023, VAL=2024, HOLDOUT=2025 -- nflverse's real weekly stats go
back to at least 2020; HOLDOUT is the most recent fully-completed season
in this pipeline's synthetic "today" (2026).

Pre-registered pass bar (all), identical to CFB's anytime_touchdowns gate:
  1. AUC >= 0.58
...
Run
---
python -u nfl_anytime_touchdowns_champion_gate_a.py
"""
...
BASELINE_DEFAULT = "nfl_models/nfl_anytime_touchdowns_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "nfl_models/nfl_anytime_touchdowns_champion_gate_a_work"
...
def load(baseline_path):
    ...
    rows = con.execute(f"SELECT {', '.join(cols)} FROM nfl_anytime_touchdowns_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("NFL_ANYTIME_TOUCHDOWNS_CHAMPION_GATE_A\n=======================================")
    ...
    verdict = ("NFL_ANYTIME_TOUCHDOWNS_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "NFL_ANYTIME_TOUCHDOWNS_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    bst.save_model(str(work / "nfl_anytime_touchdowns.json"))
    (work / "nfl_anytime_touchdowns_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "NFL_ANYTIME_TOUCHDOWNS_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
    ...
    (work / "nfl_anytime_touchdowns_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

...
Validates the SAME cfb_rush_sim.simulate() function (no new math -- the
two-stage bootstrap, sample event count from a recent real game then
sample each event's yards from a pooled recent-outcome set, is identical
whether the "event" is a carry or a catch) against the untouched 2025 WR
receiving_yards holdout. Same population/eligibility as the shipped
classifier (cfb_receiving_yards_champion_gate_e2.py: WR, Power4-vs-
Power4 only, >=3 prior games, recent3 receptions >= 5), so directly
comparable to its validated holdout AUC of 0.6374.
...
Run
---
python -u cfb_recv_sim_gate_a.py
"""
import sqlite3
import sys
...
def build_asof_rows(model_con, carry_con, season):
    ...
    rows = model_con.execute("""
        SELECT player_id, player_name, team, opponent, week, game_id, receptions, receiving_yards
        FROM player_games WHERE position='WR' AND season=?
        ORDER BY player_id, week, game_date
    """, (season,)).fetchall()

    catch_rows = carry_con.execute("""
        SELECT player_id, week, game_id, yards FROM recv_catches WHERE season=?
        ORDER BY player_id, week, game_id, catch_index
    """, (season,)).fetchall()
    by_player_game = {}
    for pid, wk, gid, yards in catch_rows:
        by_player_game.setdefault((pid, wk, gid), []).append(yards)

    out = []
    cur_pid = None
    hist = []
    for pid, pname, team, opp, week, gid, receptions, rec_yards in rows:
        if pid != cur_pid:
            cur_pid = pid
            hist = []
        if gid in p4_games:
            ...
            if n_prior >= MIN_PRIOR_GAMES_FOR_RATE and recent_rate >= MIN_RECENT_RECEPTIONS_PER_GAME:
        ...
        hist.append((week, receptions if receptions is not None else 0, this_game_yards))
    return out


def main():
    print("CFB_RECV_SIM_GATE_A\n====================")
    ...
    print(f"holdout {HOLDOUT_SEASON}: {len(rows)} eligible WR rows (Power4-vs-Power4 only)")
    if not rows:
    ...
    print("(shipped classifier's validated holdout AUC for comparison: 0.6374)")

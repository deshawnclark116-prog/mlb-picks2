#!/usr/bin/env python3
"""
CFB_FCS_PASSING_YARDS_CLEAN_BASELINE_A

FCS analog of the FBS passing_yards baseline (QB population, margin
features). Same DEV/VAL/HOLDOUT structure as the other FCS markets
(DEV=2022-2023, VAL=2024, HOLDOUT=2025). MIN_RECENT_ATTEMPTS_PER_GAME
kept at FBS's own value (15). LINE derived fresh.

Run
---
python -u cfb_fcs_passing_yards_clean_baseline_a.py
"""
...
WORKDIR_DEFAULT = "/data/cfb_model/cfb_fcs_passing_yards_clean_baseline_a_work"
MIN_PRIOR_GAMES_FOR_RATE = 3
MIN_RECENT_ATTEMPTS_PER_GAME = 15
THRESHOLD_CANDIDATES = [124.5, 154.5, 184.5, 214.5, 244.5, 274.5]
...
def main():
    ...
    print("CFB_FCS_PASSING_YARDS_CLEAN_BASELINE_A\n=======================================")
    print(f"source={src}\nworkdir={work}")
    ...
    dev_py = [r["actual_passing_yards"] for r in rows if r["season"] in DEV_SEASONS]
    print(f"\nTHRESHOLD DIAGNOSTIC (dev {DEV_SEASONS} only, n={len(dev_py)})")
    print(f"  {'threshold':>10s}{'over_rate':>12s}")
    best_t, best_dist = None, 1.0
    for t in THRESHOLD_CANDIDATES:
        rate = sum(1 for y in dev_py if y >= t + 0.5) / len(dev_py) if dev_py else 0
    ...
    out.execute(f"""CREATE TABLE cfb_fcs_passing_yards_baseline (
    ...
    ins = f"INSERT INTO cfb_fcs_passing_yards_baseline ({', '.join(insert_cols)}) VALUES ({placeholder})"
    ...
    manifest = {
        "script": "CFB_FCS_PASSING_YARDS_CLEAN_BASELINE_A",

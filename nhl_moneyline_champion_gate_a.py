#!/usr/bin/env python3
"""
NHL_MONEYLINE_CHAMPION_GATE_A

Real team-level "who wins" classifier for NHL, mirroring the same proven
...
DEV=2018-2022, VAL=2023, HOLDOUT=2024 -- same split convention as CFB's
moneyline gate. 2025 is reserved for live serving's prior-season
fallback (see nhl_prior_season_moneyline_gate_a.py).

Pre-registered pass bar (all), identical to every other market's gate:
  1. AUC >= 0.58
...
Run
---
python -u nhl_moneyline_champion_gate_a.py
"""
...
BASELINE_DEFAULT = "nhl_models/nhl_moneyline_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "nhl_models/nhl_moneyline_champion_gate_a_work"
...
def load(baseline_path):
    ...
    rows = con.execute(f"SELECT {', '.join(cols)} FROM nhl_moneyline_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("NHL_MONEYLINE_CHAMPION_GATE_A\n=============================")
    ...
    verdict = ("NHL_MONEYLINE_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "NHL_MONEYLINE_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    bst.save_model(str(work / "nhl_moneyline.json"))
    (work / "nhl_moneyline_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "NHL_MONEYLINE_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
    ...
    (work / "nhl_moneyline_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

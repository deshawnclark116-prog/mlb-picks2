#!/usr/bin/env python3
"""
HR SQLite Dataset Analysis A

Analyzes the leak-safe SQLite HR model dataset built by:
- hr_sqlite_foundation_a.py
- hr_statcast_bbe_loader_a.py
- hr_sqlite_feature_builder_a.py

This analyzer is built for the new suffixed rolling features:
    batter_barrel_rate_15d
    batter_hard_hit_rate_30d
    pitcher_barrel_allowed_rate_60d
etc.

Run:
    python hr_sqlite_dataset_analysis_a.py --min-n 5

More conservative:
    python hr_sqlite_dataset_analysis_a.py --min-n 20 --min-bbe 5
"""
...
def main() -> int:
    ...
    ap.add_argument("--min-bbe", type=int, default=3)
    ...
    print("HR SQLITE DATASET ANALYSIS A")
    print("============================")
    print(f"db: {db}")
    print(f"rows: {n}")
    print(f"hr_hits: {h}")
    ...
    feature_fields = []
    for r in rows[:1]:
        feature_fields = [k for k in r.keys() if ("_15d" in k or "_30d" in k or "_60d" in k)]
    ...
    for field in sorted(feature_fields):
        ...
        print(f"{field:42s} have={have:4d}/{n:<4d} nonzero={nonzero:4d}/{n:<4d}")
    ...
    print("--------------------")
    key_avg_fields = [
        "batter_barrel_rate_15d", "batter_barrel_rate_30d", "batter_barrel_rate_60d",
        "batter_hard_hit_rate_15d", "batter_hard_hit_rate_30d", "batter_hard_hit_rate_60d",
        "batter_la_20_35_rate_15d", "batter_la_20_35_rate_30d", "batter_la_20_35_rate_60d",
        "batter_fly_ball_rate_15d", "batter_fly_ball_rate_30d", "batter_fly_ball_rate_60d",
        "batter_avg_ev_15d", "batter_avg_ev_30d", "batter_avg_ev_60d",
        "batter_max_ev_15d", "batter_max_ev_30d", "batter_max_ev_60d",
        "pitcher_barrel_allowed_rate_15d", "pitcher_barrel_allowed_rate_30d", "pitcher_barrel_allowed_rate_60d",
        "pitcher_fly_ball_rate_15d", "pitcher_fly_ball_rate_30d", "pitcher_fly_ball_rate_60d",
        "pitcher_hrfb_rate_15d", "pitcher_hrfb_rate_30d", "pitcher_hrfb_rate_60d",
    ]
    for field in key_avg_fields:
        ...
        print(f"{field:42s} hit_avg={ah} miss_avg={am} delta={delta}")
    ...
    for field, ths in thresholds.items():
        if field not in feature_fields:
            continue
        # sample field corresponding to batter/pitcher sample size
    ...
    for w in ("15d", "30d", "60d"):
        combos.extend([
            (f"{w}: barrel>=.10 + hardhit>=.45", [ge(f"batter_barrel_rate_{w}", .10), ge(f"batter_hard_hit_rate_{w}", .45), min_sample(f"batter_bbe_{w}", args.min_bbe)]),
            ...
            (f"{w}: batter_hardhit>=.45 + pitcher_hrfb>=.12", [ge(f"batter_hard_hit_rate_{w}", .45), ge(f"pitcher_hrfb_rate_{w}", .12), min_sample(f"batter_bbe_{w}", args.min_bbe), min_sample(f"pitcher_bbe_allowed_{w}", args.min_bbe)]),
        ])
    ...
    print("\nREAD")
    print("----")
    print("This is the correct analysis layer for the SQLite pipeline.")
    print("If feature availability is 0 or mostly zero, the target batter_games do not have enough prior BBE lookback loaded.")
    print("For a target game_date of 2026-06-28, strict leakage-safe features need BBE data before 2026-06-28.")
    print("Next best step: import more batter_games from dates after the loaded BBE range OR load earlier BBE lookback.")
    conn.close()
    return 0

#!/usr/bin/env python3
"""
CFB_PASSING_YARDS_CHAMPION_GATE_A

First honest CFB passing-yards model, direct binary:logistic classifier.
Predictions-first: no odds. QB-only population (see clean_baseline).

Starts from this pipeline's already-proven configuration rather than
re-discovering it: margin-context features (team_net_margin/opp_net_
margin/projected_margin) and regularized hyperparameters (max_depth=3,
min_child_weight=15, reg_lambda=3.0, eta=0.03) both came from real,
already-validated improvements on rushing_yards -- applying them from the
start here, not re-running the "no margin features" failure a third time.
Calibration is tested via the bootstrap goodness-of-fit p-value (not a
fixed ECE number) for the same reason already established and reused
across every market since rushing_yards: a null-simulation proved a fixed
ECE<=0.02 bar is statistically unachievable at holdout sizes this small,
regardless of model quality -- reusing the fix, not inventing a new one.
...
Run
---
python -u cfb_passing_yards_champion_gate_a.py
"""
...
BASELINE_DEFAULT = "/data/cfb_model/cfb_passing_yards_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "/data/cfb_model/cfb_passing_yards_champion_gate_a_work"
...
def main():
    ...
    print("CFB_PASSING_YARDS_CHAMPION_GATE_A\n==================================")
    ...
    report = {"script": "CFB_PASSING_YARDS_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,

#!/usr/bin/env python3
"""
CFB_RUSHING_YARDS_WALKFORWARD_STABILITY_A

Final rung before live wiring, mirroring nfl_defense_sacks_walkforward_
stability_a.py exactly (same two-rung structure, same bars). Uses the
exact model/features/hyperparameters gate_e already validated (imports
from cfb_rushing_yards_champion_gate_d, gate_e's underlying module) --
this script does not retrain a different model, it re-examines the
already-passing predictions for consistency.

RUNG 1 -- walk-forward calibration (deployment-shaped test): as each 2025
week finishes, refit a 2-parameter Platt map on the 2024 internal-val
slice + 2025 weeks seen so far (strictly earlier only), predict the NEXT
week with it -- exactly how a live 'growing' calibration_policy would
behave. Note: gate_e's RAW (uncalibrated) probabilities already passed the
bootstrap calibration test cleanly (p=0.26) -- this rung checks whether
adding weekly Platt on top holds up too, not because raw failed.
...
Run
---
python -u cfb_rushing_yards_walkforward_stability_a.py
"""
import json
import sys
...
WORKDIR = Path("/data/cfb_model/cfb_rushing_yards_walkforward_stability_a_work")
...
def main():
    ...
    print("CFB_RUSHING_YARDS_WALKFORWARD_STABILITY_A\n" + "=" * 42)
    ...
    print(f"warmup calibration pool: 2024 internal val, n={len(va)}")
    ...
    verdict = ("CFB_RUSHING_YARDS_WALKFORWARD_STABLE_READY_FOR_LIVE_WIRING" if stable
               else "CFB_RUSHING_YARDS_WALKFORWARD_NOT_YET_STABLE")
    print(f"\n================ FINAL VERDICT ================\n  {verdict}")

    bst.save_model(str(WORKDIR / "cfb_rushing_yards.json"))
    (WORKDIR / "cfb_rushing_yards_columns.json").write_text(json.dumps(g.FEATURES))
    report = {
        "script": "CFB_RUSHING_YARDS_WALKFORWARD_STABILITY_A",
    ...
    (WORKDIR / "cfb_rushing_yards_walkforward_stability_a_report.json").write_text(json.dumps(report, indent=2))

#!/usr/bin/env python3
"""
CFB_FCS_RUSHING_YARDS_CHAMPION_GATE_E

Runs against clean_baseline_c: same population/eligibility/features as
gate_a, but scoped to games where BOTH teams are from a scholarship-
awarding FCS conference (excludes Ivy/Pioneer, the two FCS conferences
that structurally never award athletic scholarships -- a real,
pre-existing structural fact, not derived from any performance data).
Same fix pattern as FBS receiving_yards/passing_yards, which needed
Power4-vs-Power4 scoping to reduce blowout-mismatch noise after model-
side corrections (Platt, isotonic, regularization, pooling -- all four
tried and failed on this market, see gate_b/c/d and the isotonic
attempt) didn't fix calibration.

Same pre-registered bar, not loosened: AUC >= 0.58, ECE <= 0.02,
logloss gain >= 0.01, Brier better than constant.

Run
---
python -u cfb_fcs_rushing_yards_champion_gate_e.py
"""
...
BASELINE_DEFAULT = "/data/cfb_model/cfb_fcs_rushing_yards_clean_baseline_c_work/baseline.sqlite"
WORKDIR_DEFAULT = "/data/cfb_model/cfb_fcs_rushing_yards_champion_gate_e_work"
...
def main():
    ...
    print("CFB_FCS_RUSHING_YARDS_CHAMPION_GATE_E\n======================================")
    ...
    report = {"script": "CFB_FCS_RUSHING_YARDS_CHAMPION_GATE_E", "holdout": HOLDOUT_SEASON,
              "constant": constant, "challenger": challenger, "gate": GATE,
              "passed": passed, "verdict": verdict, "importance": imp,
              "best_iteration": bst.best_iteration}
    (work / "cfb_fcs_rushing_yards_champion_gate_e_report.json").write_text(json.dumps(report, indent=2))

#!/usr/bin/env python3
"""
NFL_DEFENSE_INTERCEPTIONS_CHAMPION_GATE_A

Predictions-first: no odds. Same gate every market this session has used.
Base rate here (~9-11%, stable) is thin -- closer to the RBI/Runs territory
(rare event, narrowly failed) than to sacks' ~33% -- so a fail here would
not be surprising, but it's tested honestly rather than presumed.

Two arms on the untouched 2025 holdout (2022-2024 = train, most recent
~20% of rows by (season, week) held out internally for early stopping):
  constant   predicts the dev-seasons base rate for every row
  challenger binary:logistic on the 7 clean-baseline features
...
Run (Render)
------------
python -u nfl_defense_interceptions_champion_gate_a.py 2>&1 | tee /data/nfl_model/nfl_defense_interceptions_champion_gate_a.log
"""
...
BASELINE_DEFAULT = "/data/nfl_model/nfl_defense_interceptions_clean_baseline_a_work/baseline.sqlite"
WORKDIR_DEFAULT = "/data/nfl_model/nfl_defense_interceptions_champion_gate_a_work"
...
def load(baseline_path):
    ...
    rows = con.execute(f"SELECT {', '.join(cols)} FROM nfl_defense_interceptions_baseline").fetchall()
    con.close()
...
def main():
    ...
    print("NFL_DEFENSE_INTERCEPTIONS_CHAMPION_GATE_A\n==========================================")
    ...
    verdict = ("NFL_DEFENSE_INTERCEPTIONS_CHAMPION_PASSES_GATE_READY_FOR_STABILITY_CONFIRMATION"
               if passed else "NFL_DEFENSE_INTERCEPTIONS_CHAMPION_DOES_NOT_CLEAR_GATE")
    ...
    bst.save_model(str(work / "nfl_defense_interceptions.json"))
    (work / "nfl_defense_interceptions_columns.json").write_text(json.dumps(FEATURES))
    report = {"script": "NFL_DEFENSE_INTERCEPTIONS_CHAMPION_GATE_A", "holdout": HOLDOUT_SEASON,
              "constant": constant, "challenger": challenger, "gate": GATE,
              "passed": passed, "verdict": verdict, "importance": imp,
              "best_iteration": bst.best_iteration}
    (work / "nfl_defense_interceptions_champion_gate_a_report.json").write_text(json.dumps(report, indent=2))

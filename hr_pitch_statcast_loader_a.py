...
def zb(px,pz):
    ...
    return 10
def key(r):
    parts=[r.get(k,"") for k in ("game_pk","game_date","at_bat_number","pitch_number","batter","pitcher","pitch_type","balls","strikes","plate_x","plate_z")]
...
def fetch(s,e):
    ...
    with urlopen(req,timeout=90) as resp:
        body=resp.read().decode("utf-8",errors="replace"); status=getattr(resp,"status",None)
    return status, list(csv.DictReader(io.StringIO(body)))
...
def main():
    ...
    print("HR_PITCH_STATCAST_LOADER_A"); print("=========================="); print("db:",a.db)
    ...
    seen=inserted=bbe=hr=0
    for s,e in chunks(dt(a.start_date),dt(a.end_date),a.chunk_days):
        print(f"\nFETCH {s} to {e}")
        try: status,rows=fetch(s,e)
